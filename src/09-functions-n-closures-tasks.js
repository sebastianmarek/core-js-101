...
function memoize(func) {
  function memo(...args) {
    ...
    memoize.memory = memoize.memory || {};

    if (memoize.memory[key]) {
      return memoize.memory[key];
    }

    memoize.memory[key] = func(...args);
    return memoize.memory[key];
  }
...
function logger(func, logFunc) {
  return (...args) => {
    logFunc(`${func.name}(${(JSON.stringify(args).substr(1, JSON.stringify(args).length - 2))}) starts`);
    logger.result = func(...args);
    logFunc(`${func.name}(${(JSON.stringify(args).substr(1, JSON.stringify(args).length - 2))}) ends`);
...
function getIdGeneratorFunction(startFrom) {
  ...
  if (!generated[startFrom]) generated[startFrom] = startFrom - 1;

  function returnNext() {
    generated[startFrom] += 1;
    return generated[startFrom];
  }
  return returnNext;
}
